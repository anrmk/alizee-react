...
export default function useSlidingViews(
  priorityView = LEFT_OPEN_TYPE,
  initView = LEFT_OPEN_TYPE
) {
    ...
    if (!up("md")) {
      if (currentState === BOTH_OPEN_TYPE) {
        ...
        setCurrentState((prev) =>
          prev === priorityView ? getView(priorityView) : priorityView
        );
      }
    } else {
      setCurrentState((prev) =>
        prev === priorityView ? BOTH_OPEN_TYPE : priorityView
      );
    }
  };

  const getView = (view) =>
    view === RIGHT_OPEN_TYPE ? LEFT_OPEN_TYPE : RIGHT_OPEN_TYPE;
