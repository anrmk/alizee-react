...
import { getErrorMaxFileSize, getErrorMaxFilesCount } from "../../constants/errors";
import { 
...
const FileUploader = React.forwardRef(({
  ...
  const [localFilters, setLocalFilters] = useState([
    file => file.size > mbSize * MEGABYTE && getErrorMaxFileSize(file.name, mbSize + "mb")
  ]);

  const handleDropdownDrop = passedFiles => {
    onItemDrop && onItemDrop(passedFiles);

    const nonDuplicateFiles = getWithoutRepeats([...files, ...passedFiles], "name");
    if (nonDuplicateFiles.length > filesCount) {
      return setErrors([getErrorMaxFilesCount(filesCount)]);
    }

    const { filteredFiles, errorsWithFiles } = getFilteredFiles(nonDuplicateFiles);
  ...
  const extendFileData = (file) => {
    ...
    if (file?.originFile) {
      fileType = file.originFile.type.substring(0, file.originFile.type.indexOf("/"));
  ...
  return (
    <section>
      <div {...getRootProps({ className: 'drop-zone ' + className, style: getStyle({ isDragActive, isDragAccept, isDragReject, files }) })}>
        <input {...getInputProps()} />
        {!files.length && <p className="m-0">Drag 'n' drop some files here, or click to select files</p>}
      ...
      {errors.length > 0 && errors.map(error => (<p className="m-0 text-danger"><small>{error}</small></p>))}
    </section>
  );
})
