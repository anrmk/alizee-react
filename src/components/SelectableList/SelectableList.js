...
export default function SelectableList({
  ...
  const fillPreSelected = (pItems, pPreSelected, pMultiple) => {
    ...
    if (pMultiple) {
      pPreSelected.forEach((key) => {
        if (typeof key !== "string" || mapWithNamesToIndex[key] === undefined)
          return;
  ...
  const isSelectedItem = (key) => !!selected[key];
  ...
  const handleItemClick = (e) => {
    ...
    if (currentKey in selected) {
      ...
      multiple ? (newSelected = filteredSelected) : (newSelected = {});
    } else {
      if (maxSelections > 0 && Object.values(selected).length >= maxSelections)
        return;

      multiple
        ? (newSelected = {
            ...selected,
            [currentKey]: transformedItems[currentKey],
          })
