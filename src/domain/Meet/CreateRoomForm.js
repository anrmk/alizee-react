...
const HELPER_TEXT_DESCRIPTION = (length) =>
  `Characters entered ${
    length || 0
  } out of ${VALUE_MAX_LENGTH_DESCRIPTION} characters`;
...
const schema = yup.object().shape({
  ...
  [TITLE_ID]: yup
    ...
    .max(
      VALUE_MAX_LENGTH_TITLE,
      VALUE_MAX_LENGTH_ERROR(VALUE_MAX_LENGTH_TITLE)
    ),
  [DESCRIPTION_ID]: yup
    .string()
    .notRequired()
    .max(
      VALUE_MAX_LENGTH_DESCRIPTION,
      VALUE_MAX_LENGTH_ERROR(VALUE_MAX_LENGTH_DESCRIPTION)
    ),
  [TAGS_ID]: yup.array().nullable().notRequired(),
  [ROOM_TYPE_ID]: yup.number().required(EMPTY_VALUE_ERROR),
  [TICKET_PRICE_ID]: yup
    .number()
    .positive()
  ...
  return (
    <Grid container direction="row" spacing={2}>
      <Grid item container direction="column" justify="center" xs={12} md={8}>
        <video
          className={classes.createRoomItemVideo}
          ref={videoRef}
          muted
          autoPlay
          playsInline
        />
        <Hidden smDown>{renderButton()}</Hidden>
      </Grid>
      ...
      <Grid
        ...
        <Paper
          elevation={0}
          className={clsx(classes.createRoomLinkId, classes.formElementIndent)}>
          <Typography
            variant="body2"
            align="center"
            noWrap
            className={classes.createRoomLinkLabel}>
        ...
        <form id={FORM_ID} onSubmit={handleSubmit(onSubmit)} autoComplete="off">
          <FormGroup className={classes.formElementHidden}>
            <Controller
              ...
              render={({ onChange, onBlur, value }) => (
                <input id={ROOM_ID} type="hidden" value={value} />
              )}
            />
          </FormGroup>
          ...
          <FormGroup
            className={clsx(
              classes.formElementIndent,
              room.title && classes.formElementHidden
            )}>
            <Controller
              name={ROOM_TYPE_ID}
              control={control}
