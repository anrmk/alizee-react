...
function Main({ 
  ...
  const handleSproutSubmit = useCallback(async (formData, mediaData) => {
    console.log("submit", formData.type)
  ...
  return (
    <Box style={!isNavigationHide ? { display: "flex" } : null}>
      ...
      {isAuthenticated && !isNavigationHide && (
        <Hidden mdUp>
          <BottomBar user={userInfo} onClick={postOnClick} onSubmit={handleSproutSubmit}  />
        </Hidden>
      )}
    </Box>
  );
